...
void Block::merge(Block &obj) {
  ...
  while (ipos < size) res[rpos++] = array_[ipos++];
  while (jpos < obj.size) res[rpos++] = obj.array_[jpos++];
...
Block Block::add(const Node &var) {
  ...
  if (targetpos == -1) targetpos = size;
  size++;
  for (int i = size - 1; i > targetpos; --i) array_[i] = array_[i - 1];
...
bool Block::dec(const Node &var) {
  ...
  size--;
  for (int i = ipos; i < size; ++i) array_[i] = array_[i + 1];
  array_[size] = Node();
  return true;
}
...
bool BlockIndex::inrange(const int &pos) { return 0 <= pos && pos < size; }

int &BlockIndex::getoffset(const int &pos) { return offset[pos]; }
